...
committed to the repo — labs download prepared slices at runtime through
`student_pack/msacl.py`, from the manifest `student_pack/datasets.json` (generated by
`tools/build_colab_bootstrap.py`; licence-cleared files hosted on Hugging Face at
`jaztsong88/msacl-ds301`).
...
- Prepared slice: `.npz` per task (~15–45 MB at float16). **Hosted** on the Hugging
  Face dataset repo `jaztsong88/msacl-ds301` (CC0 allows it); labs fetch it through
  `msacl.setup()` with a SHA-256 check (DECISIONS.md 2026-09-23).
...
- Prep script: `data/prep/prepare_peakonly.py` — **tested end-to-end**: resamples
  ...
  `data/slices/peakonly_roi_qc.npz`. **Not rehosted yet** — the annotation zip
  carries no licence; `publish=False` in `tools/build_colab_bootstrap.py` until cleared
  (Lab 5 Track B degrades gracefully without it).
...
- Hosted slice for Colab download: (entry in `DATASETS`, `tools/build_colab_bootstrap.py`)
