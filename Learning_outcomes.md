...
- Explain, from a live three-stage build, why **memory is not knowledge**: a bare LLM forgets the run you just named; the same LLM with conversation memory answers that correctly and then *invents* a mass error, passing a run that fails all three limits; only tools ground it. Say what changes at each step.
- State the two rules the demo makes concrete: the **script owns the tools** (the model only emits text, so "restricted tools" is enforceable rather than a promise), and an agent **only knows what its context says exists** (the system prompt must inventory the data, or the agent guesses).
...
- 0:22–0:40 **Instructor live demo — building an agent in three steps** (restructured 2026-09-14; was a storyboard of one finished assistant). A step-by-step walkthrough of the three scripts in `demos/lecture14_agent/`, each adding exactly ONE idea and each failing in a new way, all asked the same three questions. Six slides: **the plan** (the three scripts against Q1 concept / Q2 memory / Q3 data); **stage 1** `stage1_chatbot.py` — the message list is rebuilt every turn, so it cannot name the run you gave it one turn ago; **stage 2** `stage2_memory.py` — the only change is that `messages` persists, Q2 now works, and then a 💬 *predict* beat before Q3, where it invents `+0.8 ppm` and **passes QC-04, the one run that fails all three limits**; **stage 3** `stage3_react.py` — the ReAct loop runs the tools and it answers `+5.6 ppm, OUT OF SPEC`, every number traceable to an Observation; **the payoff** — `draft_summary` names the run, its out-of-range values and "please verify before releasing any results"; and **what made it work** — the script owns the tools, and the agent only knows what its context says exists. Slide panels reuse the demo's own terminal chips so the deck and the projector match. (Fallback: a saved `tee`'d transcript — the demo never fails live.)
