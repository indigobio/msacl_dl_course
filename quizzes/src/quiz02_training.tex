...
% =========================== Q4 ===========================
\textbf{4. Put the training loop in order.}\; {\small Number these five lines \textbf{1--5} in the
order they run each iteration (forward $\rightarrow$ loss $\rightarrow$ zero grad $\rightarrow$
backward $\rightarrow$ step).}

\vspace{4pt}
\begin{center}
\begin{tabular}{@{}c@{\quad}l@{\qquad}p{0.42\textwidth}@{}}
\rb{$4$}{1cm} & \texttt{loss.backward()}        & {\small\itshape backprop the blame} \\[5pt]
\rb{$1$}{1cm} & \texttt{pred = model(x)}         & {\small\itshape forward --- make a guess} \\[5pt]
\rb{$5$}{1cm} & \texttt{optimizer.step()}        & {\small\itshape nudge every weight} \\[5pt]
\rb{$2$}{1cm} & \texttt{loss = loss\_fn(pred, y)}& {\small\itshape how wrong?} \\[5pt]
\rb{$3$}{1cm} & \texttt{optimizer.zero\_grad()}  & {\small\itshape clear old gradients} \\
\end{tabular}
\end{center}

\work{0.3cm}

\keybox{Correct order: \textbf{1}~\texttt{pred = model(x)}; \textbf{2}~\texttt{loss =
loss\_fn(pred, y)}; \textbf{3}~\texttt{optimizer.zero\_grad()}; \textbf{4}~\texttt{loss.backward()};
\textbf{5}~\texttt{optimizer.step()}. The \texttt{zero\_grad()} step matters because PyTorch
\emph{adds up} gradients --- without clearing them first, last iteration's gradients pile onto
this one's and corrupt the step.}
